...
export default function Savers() {
  ...
  const filtered = useMemo(() => {
    return savers.filter((saver) => {
      const matchesSearch =
        saver.name.toLowerCase().includes(search.toLowerCase()) ||
        saver.email.toLowerCase().includes(search.toLowerCase());

      const matchesStatus = status === "All" || saver.status === status;
  ...
  const stats = {
    total: savers.length,
    active: savers.filter((s) => s.status === "Active").length,
    pending: savers.filter((s) => s.status === "Pending").length,
    suspended: savers.filter((s) => s.status === "Suspended").length,
  };
  ...
  return (
    <div className="space-y-8 mb-4 w-full px-0 sm:px-0 lg:px-2">
      {/* PAGE TITLE */}
      {/* <div className="rounded-xl border bg-white p-4">
        <h2 className="  text-2xl font-bold text-black">Saver Management</h2>
        <p className="mt-1 text-gray-600">Manage all savers.</p>
      </div> */}
      {/* STATS */}
      <section className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4 sm:-mt-6">
        <div className="rounded-xl bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between">
            <p className="text-[14px] text-gray-600">Total Savers</p>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[900px] w-full">
            ...
            <tbody>
              {filtered.map((saver) => (
                ...
                >
                  <td className="px-6 py-5">
                    <p className="font-semibold text-gray-600">{saver.name}</p>

                    <p className="text-sm text-gray-500">{saver.email}</p>
                  </td>

                  <td className="px-6 py-5 text-gray-600">{saver.phone}</td>

                  <td className="px-6 py-5 text-gray-600">{saver.referrals}</td>
          ...
          <section className="grid grid-cols-1 gap-6 xl:grid-cols-3">
            ...
            <div className="rounded-xl bg-white p-6 shadow-sm">
              ...
              <div className="space-y-4">
                {savers
                  .filter((saver) => saver.status === "Pending")
                  .map((saver) => (
                    <div key={saver.id} className="rounded-xl border p-4">
                      <h4 className="font-semibold text-gray-600">
                        {saver.name}
                      </h4>
            ...
            <div className="rounded-xl bg-white p-6 shadow-sm">
              ...
              <div className="space-y-5">
                <div className="border-l-4 border-primary pl-4">
                  <p className="font-medium text-gray-600">
                    Adaeze Okafor created a Fixed Savings Plan
                  </p>
                ...
                <div className="border-l-4 border-primary pl-4">
                  <p className="font-medium text-gray-600">
                    Tunde Bello submitted KYC documents
                  </p>
                ...
                <div className="border-l-4 border-primary pl-4">
                  <p className="font-medium text-gray-600">
                    Ifeoma Nwosu referred 12 savers
                  </p>
